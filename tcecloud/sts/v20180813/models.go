...
type AssumeRoleRequest struct {
	*tchttp.BaseRequest
	...
	// 策略描述注意：1、policy 需要做 urlencode（如果通过 GET 方法请求云 API，发送请求前，所有参数都需要按照[云 API 规范](/ocloud/api/TCenter_CAT/TCENTER_SUBCAT/APIs/密钥服务（sts）/版本（2018-08-13）/调用方式/接口签名v3)再 urlencode 一次）。2、策略语法参照CAM策略语法。3、策略中不能包含 principal 元素。
